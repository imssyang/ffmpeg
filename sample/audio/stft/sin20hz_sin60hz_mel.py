...
def main():
    ...
    axs[1].set(title='STFT Spectrogram (in dB)')
    axs[1].xaxis.set_label_coords(1.0, -0.13)
    img_1 = librosa.display.specshow(S_db, sr=sample_rate, hop_length=hop_length, x_axis='s', y_axis='log', ax=axs[1])
    plt.colorbar(img_1, ax=axs[1], format="%+2.0f dB")

    axs[2].set(title=f'Mel Filterbank ({"HTK" if htk else "Slaney"})')
    axs[2].xaxis.set_label_coords(1.0, -0.13)
    ax2_x = np.arange(Mel_matrix.shape[1]) * sample_rate / n_fft
    axs[2].plot(ax2_x, Mel_matrix.T)

    axs[3].set(title='Mel Spectrogram (in dB)')
    axs[3].xaxis.set_label_coords(1.0, -0.13)
    img_3 = librosa.display.specshow(Mel_db, sr=sample_rate, hop_length=hop_length, x_axis='s', y_axis='mel', htk=htk, ax=axs[3])
    plt.colorbar(img_3, ax=axs[3], format="%+2.0f dB")
